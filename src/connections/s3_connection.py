...
class s3_operations:
    def __init__(self, bucket_name, region_name="us-east-1"):
        """
        Initialize the s3_operations class using IAM role credentials.
        No access key or secret key required.
        """
        self.bucket_name = bucket_name
        self.s3_client = boto3.client('s3', region_name=region_name)

        logging.info(f"S3 operations initialized for bucket: {bucket_name}")

    def fetch_file_from_s3(self, file_key):
        """
        Fetch a CSV file from the S3 bucket and return it as a Pandas DataFrame.
        ...
        try:
            ...
            df = pd.read_csv(obj['Body'])  # boto3 returns a stream-like object
            
            logging.info(f"✅ Successfully loaded '{file_key}' with {len(df)} records.")
            return df
